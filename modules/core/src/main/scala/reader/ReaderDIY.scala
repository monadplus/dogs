...
object ReaderDIY extends App {
  ...
  object Reader {
    private[Reader] final class PurePartiallyApplied[A](
  ...
  def updatePassword(userId: String,
                     newPassword: Password): Reader[Connection, Unit] = ???
  def changePwd(userId: String,
                oldPwd: Password,
                newPwd: Password): Reader[Connection, Boolean] =
    for {
